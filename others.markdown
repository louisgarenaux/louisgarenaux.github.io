...
- *Intersection between control theory and game theory.* Consider the two dimensional heat equation with a forcing term on an open region. Assume the forcing term is the sum of two contributions, each of which is chosen by one player. What is the dynamic when:
